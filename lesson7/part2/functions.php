...
function show_ads_list() {
    foreach (glob('./ads/*.txt') as $ad) {
        $ad_data = unserialize(file_get_contents($ad));
        echo "<tr>
                  <td><a href='?id=" . substr($ad, -10, 6) . "'>{$ad_data['title']}</a></td>
                  <td>{$ad_data['price']}</td>
                  <td>{$ad_data['seller_name']}</td>
                  <td><a href='#' data-href='?delete=" . substr($ad, -10, 6) . "' data-toggle='modal' data-target='#confirm-delete'>Удалить</a></td>
              </tr>";
    }
}

function show_form($id = '') {
    if ($id) $ad = unserialize(file_get_contents('./ads/' . $id . '.txt'));
