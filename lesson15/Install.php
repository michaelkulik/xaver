...
class Install
{
    public static function setDump(Smarty $smarty)
    {
        ...
        $db->query('SET foreign_key_checks = 0'); // без этого, если есть FOREIGN KEY, может не произойти удаление таблиц перед дампом
    ...
    (8, \'Мошково\', \'641760\')';
        ...
        if ($db->query($dump)) { // выполнение запроса с дампом
            $smarty->assign('success', true);
        } else {
            $smarty->assign('error', 'Не удалось сделать дамп базы данных');
        }

        $db->query('SET foreign_key_checks = 1'); // возвращаем значение foreign_key_checks на прежнее
    }
}


if (isset($_POST['submit'])) {
    Install::setDump($smarty);
}
